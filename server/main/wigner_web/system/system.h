...
namespace wigner_web::system{
    ...
    public:
        ...
        std::shared_ptr<wigner_web::map::MapWaveFunction> create_operator_wavefunction(std::shared_ptr<wigner_web::discretization::Basis> basis, int left_derivative, int right_derivative, std::function<std::complex<double>(double)> V, int order) const;
        std::shared_ptr<wigner_web::propagation::DiagonalPropagator> create_wavefunction_propagator(std::string name, std::shared_ptr<wigner_web::map::MapWaveFunction> map) const;
